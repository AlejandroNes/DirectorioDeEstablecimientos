...
<div class="wrapper">
	...
	if(!empty($main_cats)) {
		?>
		<div class="full-block" style="background-color: #2c3e50;">
			<div class="home-main-cats" style="background-color: #34495e;">
				<?php
				$i = 1;
				foreach($main_cats as $v) {
					?>
					<div style="margin-top: 20px;" class="main-cat-block block-<?= $i; ?>">
						<div class="main-cat-icon">
							<a style="background-color: white; border-radius: 50%; padding: 10px;" href="<?= $baseurl; ?>/<?= $loc_slug; ?>/list/<?= $v['cat_slug']; ?>/<?= $loc_type; ?>-<?= $loc_id; ?>-<?= $v['cat_id']; ?>-1">
							<?= $v['iconfont_tag']; ?>
							</a>
						</div>
						<div class="main-cat-name" >
							<a style="color: white;" href="<?= $baseurl; ?>/<?= $loc_slug; ?>/list/<?= $v['cat_slug']; ?>/<?= $loc_type; ?>-<?= $loc_id; ?>-<?= $v['cat_id']; ?>-1">
							<?= $v['cat_name']; ?>
							</a>
						</div>
					</div>
