...
// arreglar ORIG_PATH_INFO
// algunos servidores definen incorrectamente ORIG_PATH_INFO igual que SCRIPT_NAME
...
try {
	...
	// por defecto es el modo de error silencioso. Cambiar para lanzar excepciones
	$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	// Deje los nombres de las columnas como los devuelve el controlador de la base de datos. Algunas extensiones de PDO los devuelven en mayúsculas
	$conn->setAttribute(PDO::ATTR_CASE, PDO::CASE_NATURAL);
	// Esto es para usar el prepare nativo, que no tiene problemas con los parámetros numéricos en la cláusula LIMIT
...
if(empty($_COOKIE['city_id'])) {
	// si no hay cookie city_id, eliminar todas las cookies relacionadas con la ciudad
	// firma: setcookie(name, value, expire, path, domain, secure, httponly);
...
// si las sesiones user_connected y userid existen, consulta la db para obtener el nombre, el apellido y el correo electrónico del usuario.
...
elseif(!empty($_COOKIE['loggedin'])) {
	...
	// generar un nuevo token e insertar el ID de usuario y el par de tokens en la base de datos
		$token = bin2hex(openssl_random_pseudo_bytes(16));
		record_tokens($cookie_userid, $cookie_provider_name, $token);

	// establecer cookie
	// firma: setcookie (nombre, valor, caducar, ruta, dominio, seguro, httponly);
		$cookie_val = "$cookie_userid-$cookie_provider_name-$token";
		setcookie('loggedin', $cookie_val, time()+86400*30, $install_path, '', '', true);
	}

	// de lo contrario, el ID de usuario y el token no existen en la base de datos
	else {
		destroy_session_and_cookie();
	}
}
