...
if($total_rows > 0) {
	...
	//seleccione toda la información de los gatos y colóquela en una matriz
	if($sort == 'sort-name') {
...
$(document).ready(function(){
	...
	$('#edit-cat-modal').on('show.bs.modal', function (event) {
		...
		var cat_id = button.data('cat-id'); // Extraer información de datos- * atributos
		var modal = $(this);
	...
	// editar formulario de gato enviar
	...
	//crear formulario de gato enviar
	...
	//crear modal de gato al cerrar
	$('#create-cat-modal').on('hide.bs.modal', function (event) {
		location.reload(true);
	});

	// inicializar la edición en su lugar
	$('.editable').jinplace();
});
</script>
</body>
